...
List<String> getUpdateFilePatterns(
...
class AutoUpdater {
  ...
  final DownloadHttpClient _downloadClient =
      DownloadHttpClient.instance;
  ...
  List<InstallationType> _detectAvailableInstallationTypes() {
    ...
    if (Platform.isLinux) {
      return [
        ...
        InstallationType.linuxTar,
      ];
    }

    if (Platform.isMacOS) {
      return [InstallationType.macosDmg];
    }

    if (Platform.isIOS) {
      return [InstallationType.ios];
    }

    if (Platform.isAndroid) {
      return [InstallationType.androidApk];
    }
    ...
    try {
      ...
      if (needUpdate(currentVersion, remoteVersion)) {
        final availableTypes =
            _detectAvailableInstallationTypes();
      ...
      KazumiLogger().e(
        'Update: check for updates failed',
        error: e,
      );
      rethrow;
    }
  ...
  Future<void> autoCheckForUpdates() async {
    final autoUpdate =
        GStorage.getSetting(SettingsKeys.autoUpdate);
    ...
    try {
      ...
      KazumiLogger().w(
        'Update: auto check for updates failed',
        error: e,
      );
    }
  }
  ...
  Future<void> _downloadUpdateWithType(
    ...
    try {
      ...
      final asset =
          getUpdateAssetForType(
        updateInfo.assets,
        selectedType,
      );

      final downloadUrl =
          getUpdateDownloadUrlFromAsset(
        asset,
      );
  ...
  Future<void> _downloadUpdate(
    ...
    if (_downloadDialogs.isRunning) {
      return;
    }

    await _downloadDialogs.run(
      (task) async {
  ...
  final _downloadDialogs =
      KazumiDialogController();
  ...
  Future<String> _downloadFile(
    ...
    KazumiLogger().i(
      'Update: file downloaded and hash verified: $filePath',
    );
  ...
  void _installUpdate(
    ...
    try {
      ...
      KazumiLogger().e(
        'Update: launch installer failed',
        error: e,
      );
    }
  }

  void _revealInFileManager(
    String filePath,
  ) async {
    try {
      final type =
      ...
      KazumiLogger().w(
        'Update: reveal in file manager failed',
        error: e,
      );
    }
  }
  ...
  String _getFileNameFromUrl(
    ...
    return 'XingFanWu-$version$extension';
  }
}
